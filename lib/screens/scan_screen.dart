...
class _ScanScreenState extends State<ScanScreen> with TickerProviderStateMixin {
  ...
  // Check if permissions were already granted, otherwise request them
  Future<void> _checkPermissionsAndStartScan() async {
    ...
    // Check if we've already been granted permissions
  ...
  // Cancel scan and clean up resources
  void _cancelScan() {
    _scanCancelled = true;
  ...
  // UI building - Main scan circle with gradient colors
    ...
    return Center(
      child: Stack(
        ...
        children: [
          ...
          // Progress circle - Using custom painter for a sleeker look
          CustomPaint(
          ...
          Column(
            ...
            children: [
              ...
              Text(
                _isScanning ? 'Scanning storage...' : 'Completed',
                style: TextStyle(
                  fontSize: 16,
                  color: _isScanning
...
Future<void> _scanDuplicateFiles(SendPort sendPort, List<Directory> dirsToScan) async {
  ...
  for (var hash in fileHashes.keys) {
    final files = fileHashes[hash]!;
    if (files.length > 1) {
